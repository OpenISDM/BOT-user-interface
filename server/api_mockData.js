...
function generate_history_record(start_time, end_time, limit_count, sort_type) {
	...
	if (end_time == undefined) end_time = moment()//.add(-1, 'days')
	else end_time = moment(end_time)
	if (start_time == undefined) start_time = moment().add(-1, 'days')
	else start_time = moment(start_time)

	if (limit_count == undefined) limit_count = 10
	if (sort_type == 'desc') {
		let start_tmp = start_time
		for (let i = 0; i < limit_count / 2; i++) {
			if (start_tmp.isAfter(moment(end_time))) break
		...
		for (let i = 0; i < limit_count / 2; i++) {
			if (end_tmp.isBefore(moment(start_time))) break
