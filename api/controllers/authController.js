...
module.exports = {
    signin: (request, response) => {
        ...
        pool.query(dbQueries.signin(username.toLowerCase()))
            .then((res) => {
                if (res.rowCount < 1) {
                    console.log(
                        `signin failed: username or password is incorrect`
                    )
                    response.json({
                    ...
                    if (hash == res.rows[0].password) {
                        ...
                        pool.query(dbQueries.setVisitTimestamp(username))
                            ...
                            .catch((err) =>
                                console.log(`set visit timestamp fails ${err}`)
                            )
                    } else {
                        response.json({
                            authentication: false,
    ...
    validation: (request, response) => {
        let { username, password, authenticatedRoles } = request.body
                ...
                if (res.rowCount < 1) {
                    ...
                    if (hash == res.rows[0].password) {
                        ...
                        if (
                            ...
                            console.log(
                                `confirm validation failed: authority is not enough`
                            )
                            response.json({
                                confirmation: false,
                        ...
                        console.log(
                            `confirm validation failed: password is incorrect`
                        )
                        response.json({
                            confirmation: false,
    ...
    sentResetPwdInstruction: (request, response) => {
        ...
        pool.query(dbQueries.validateEmail(email))
            .then((res) => {
                if (res.rowCount != 0) {
                    let { id, password, registered_timestamp } = res.rows[0]
                    var token = jwt.sign(
                        {
                    ...
                    mailTransporter
                        ...
                        .then((res) => {
                            console.log(
                                'send password reset instruction succeed'
                            )
                            response.status(200).json()
                        })
                        .catch((err) => {
                            console.log(
                                `send password reset instruction failed ${err}`
                            )
                        })
                } else {
