...
const getTrackingData = (request, response) => {
    ...
    let { user, areaId, key } = request.body
    ...
    pool.query(dbQueries.getTrackingData(userAuthenticatedAreasId, key))
        .then((res) => {
            ...
            const toReturn = res.rows.map((item, index) => {
                ...
                let lbeacon_coordinate = item.lbeacon_uuid
                ...
                let lbeaconAreaId = lbeacon_coordinate
                    ? lbeacon_coordinate[2]
                    : null

                let isLbeaconMatchArea = lbeaconAreaId == currentAreaId

                let isUserSObject = userAuthenticatedAreasId.includes(
