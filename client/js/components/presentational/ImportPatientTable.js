			...
			try {
				...
				data.map((importData) => {
					...
					this.state.data.map((dataOrigin) => {
						importData.asset_control_number == dataOrigin.asset_control_number
							? (reapetFlag = true)
							: null
					})
					if (reapetFlag == false) {
				...
				if (punctuationFlag) {
					...
					await apiHelper.importedObjectApiAgent.addImportedObject({
						locale: locale.abbr,
						newData,
					})
					this.handleSubmitForm()
				...
				<div className="d-flex justify-content-between">
					...
					>
						<ButtonToolbar>
							...
							>
								<PrimaryButton className="mr-2 mb-1">
